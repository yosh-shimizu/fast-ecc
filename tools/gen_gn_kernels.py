# Emit the four fused Gauss-Newton kernels of src/gn_fused.inc as straight-line
...
out = []
for name, K, L, U, cols in MOTIONS:
    P = len(cols)
    kp = tri(K)
    lp = tri(L)
    ...
    # For L == 3 those are (fy,fy), (fy,1) and (1,1); the last is the one that
    # gets accumulated, the first two are it scaled by fy^2 and fy.
    ONE = lp.index((L - 1, L - 1))                 # the w = 1 column
    rowconst = [b for b, (i, j) in enumerate(lp) if i != 0 and b != ONE] if L == 3 else []

    acc = ['m%d_%d' % (a, b) for a in range(len(kp)) for b in range(len(lp))]
    live = [a for a in acc if int(a.split('_')[1]) not in rowconst]

    # a projection column carrying w = fy is the matching w = 1 column scaled.
    # Where that partner column is not in the list (homography's (e, 1)), one
    # extra accumulator stands in for it.
    ...
    o = []
    o.append('// ---- %s: K=%d L=%d P=%d, %d Gram sums + %d projection sums ----'
             % (name.lower(), K, L, P, len(acc), 2 * P))
    o.append('struct GN%s {' % name)
    o.append('    enum { P = %d };' % P)
    o.append(MEMBERS[name].rstrip('\n') if MEMBERS[name] else '')
    ...
    if L == 3:
        ...
        for idx, (i, j) in enumerate(lp):
            if idx in rowconst or idx == ONE:
                continue           # constant along the row, or the bare sum
